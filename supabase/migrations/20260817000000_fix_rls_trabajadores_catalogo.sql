...
-- Elimina cualquier política existente en la tabla
DO $$
DECLARE pol record;
BEGIN
...
-- Política: cada usuario autenticado solo ve trabajadores de su org
CREATE POLICY "trabajadores_select_org"
  ON trabajadores FOR SELECT TO authenticated
  USING (org_id = get_user_org_id());

-- Política: solo ADMIN y SUPERVISOR pueden insertar/modificar (canWrite)
