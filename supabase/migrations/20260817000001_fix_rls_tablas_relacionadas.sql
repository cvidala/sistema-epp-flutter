...
-- Estas tablas no tienen org_id directo — se filtran via obra_id → obras.org_id

-- ── Helper ya existe desde la migración anterior ─────────────
-- get_user_org_id() definida en 20260817000000
...
CREATE POLICY "trabajador_obras_select_org"
  ...
  USING (
    obra_id IN (
      SELECT obra_id FROM obras WHERE org_id = get_user_org_id()
    )
  );
...
CREATE POLICY "trabajador_obras_write_org"
  ...
  USING (
    obra_id IN (SELECT obra_id FROM obras WHERE org_id = get_user_org_id())
  )
  WITH CHECK (
    obra_id IN (SELECT obra_id FROM obras WHERE org_id = get_user_org_id())
  );
...
-- INSERT: permitido por authenticated (la app usa JWT de usuario logueado)
